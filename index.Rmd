...
st <- get_urbn_map("states", sf = TRUE)
st1 <- get_urbn_map("territories_states", sf = TRUE)
...
geographic_data <- left_join(states, statedata, by = "state_name")
...
ggplot() +
  geom_sf(data = geographic_data,
          mapping = aes(geometry = geometry, fill = medhhincome),
          color = "white") +
...
```{r ccdf, echo = FALSE, out.width = "50%"}
